...
$(document).ready(function () {
    // Делегирование событий для кнопки в форме
    ...
    function validateEmail() {
        ...
        if (emailValue === "" || !emailRegex.test(emailValue)) {
            emailInput[0].setCustomValidity("Пожалуйста, введите корректный адрес электронной почты");
            return false;
        } else {
    ...
    function validatePassword() {
        var passInput = $("#pass");
        var passValue = passInput.val().trim();

        if (passValue === "") {
            passInput[0].setCustomValidity("Пожалуйста, введите пароль");
            return false;
        } else {
    ...
    // Добавляем проверку в конце, чтобы убедиться, что скрипт выполняется
